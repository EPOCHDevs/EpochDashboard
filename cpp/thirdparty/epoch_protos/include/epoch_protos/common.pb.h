...
enum DashStyle : int {
  ...
  LongDashDot = 10,
  LongDashDotDot = 11,
  DashStyle_INT_MIN_SENTINEL_DO_NOT_USE_ =
      std::numeric_limits<::int32_t>::min(),
  DashStyle_INT_MAX_SENTINEL_DO_NOT_USE_ =
      std::numeric_limits<::int32_t>::max(),
};

bool DashStyle_IsValid(int value);
extern const uint32_t DashStyle_internal_data_[];
constexpr DashStyle DashStyle_MIN = static_cast<DashStyle>(0);
constexpr DashStyle DashStyle_MAX = static_cast<DashStyle>(11);
constexpr int DashStyle_ARRAYSIZE = 11 + 1;
const ::google::protobuf::EnumDescriptor*
DashStyle_descriptor();
template <typename T>
const std::string& DashStyle_Name(T value) {
  static_assert(std::is_same<T, DashStyle>::value ||
                    std::is_integral<T>::value,
                "Incorrect type passed to DashStyle_Name().");
  return DashStyle_Name(static_cast<DashStyle>(value));
}
template <>
inline const std::string& DashStyle_Name(DashStyle value) {
  return ::google::protobuf::internal::NameOfDenseEnum<DashStyle_descriptor,
                                                 0, 11>(
      static_cast<int>(value));
}
inline bool DashStyle_Parse(absl::string_view name, DashStyle* value) {
  return ::google::protobuf::internal::ParseNamedEnum<DashStyle>(
      DashStyle_descriptor(), name, value);
}
...
template <>
struct is_proto_enum<::epoch_proto::DashStyle> : std::true_type {};
template <>
inline const EnumDescriptor* GetEnumDescriptor<::epoch_proto::DashStyle>() {
  return ::epoch_proto::DashStyle_descriptor();
}
