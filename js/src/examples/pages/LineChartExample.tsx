    ...
    <div className="min-h-screen bg-secondary-mildCementGrey p-8">
      ...
      <div className="bg-primary-white/2 border border-primary-white/10 rounded-lg p-6">
        ...
        <div className="space-y-4">
          ...
          {chartConfigs[selectedChart].data.yPlotBands && (
            <div>
              <h4 className="text-sm font-medium text-secondary-ashGrey mb-2">Plot Bands</h4>
              <pre className="bg-black/20 p-4 rounded text-xs text-primary-white overflow-x-auto">
                {JSON.stringify(chartConfigs[selectedChart].data.yPlotBands, null, 2)}
              </pre>
            </div>
          )}

          {chartConfigs[selectedChart].data.overlay && (
            <div>
              <h4 className="text-sm font-medium text-secondary-ashGrey mb-2">Overlay Configuration</h4>
              <pre className="bg-black/20 p-4 rounded text-xs text-primary-white overflow-x-auto">
                {JSON.stringify({
                  ...chartConfigs[selectedChart].data.overlay,
                  data: `${chartConfigs[selectedChart].data.overlay.data?.length || 0} points`
                }, null, 2)}
              </pre>
            </div>
          )}
