    ...
    <div className="min-h-screen bg-secondary-mildCementGrey p-8">
      ...
      <div className="bg-primary-white/2 border border-primary-white/10 rounded-lg p-6">
        ...
        <div className="space-y-4">
            ...
            <pre className="bg-black/20 p-4 rounded text-xs text-primary-white overflow-x-auto max-h-64 overflow-y-auto">
              {JSON.stringify(
                chartConfigs[selectedChart].data.data?.map(series => ({
                  ...series,
                  values: `[${series.values?.length || 0} data points]`,
                  sampleValues: series.values?.slice(0, 5).map(v => v.toFixed(2)) || []
                })),
                null,
                2
              )}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 bg-primary-white/10 text-xs text-primary-white rounded">
                Data Points: {chartConfigs[selectedChart].data.data?.[0]?.values?.length || 0}
              </span>
              ...
              {chartConfigs[selectedChart].data.showStatistics && (
                <span className="px-2 py-1 bg-primary-white/10 text-xs text-primary-white rounded">
                  Statistics Enabled
                </span>
              )}
              ...
              {chartConfigs[selectedChart].data.color && (
                <span
                  className="px-2 py-1 text-xs text-primary-white rounded"
                  style={{ backgroundColor: chartConfigs[selectedChart].data.color }}
                >
                  Custom Color
                </span>
              )}
          ...
          {/* Statistical Summary */}
          {chartConfigs[selectedChart].data.data?.[0]?.values && (
            <div>
              <h4 className="text-sm font-medium text-secondary-ashGrey mb-2">Statistical Summary</h4>
              <div className="bg-black/20 p-4 rounded">
                {(() => {
                  const values = chartConfigs[selectedChart].data.data?.[0]?.values || []
