      ...
      <div className="bg-primary-white/2 border border-primary-white/10 rounded-lg p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <pre className="bg-black/20 p-4 rounded text-xs text-primary-white overflow-x-auto">
              {JSON.stringify({
                dataPoints: chartConfigs[selectedChart].data.data?.length || 0,
          ...
          {chartConfigs[selectedChart].data.colorAxis && (
            <div>
              <h4 className="text-sm font-medium text-secondary-ashGrey mb-2">Color Configuration</h4>
              <pre className="bg-black/20 p-4 rounded text-xs text-primary-white overflow-x-auto">
                {JSON.stringify(chartConfigs[selectedChart].data.colorAxis, null, 2)}
              </pre>
            </div>
          )}

          {chartConfigs[selectedChart].data.dataLabels && (
            <div>
              <h4 className="text-sm font-medium text-secondary-ashGrey mb-2">Data Labels</h4>
              <pre className="bg-black/20 p-4 rounded text-xs text-primary-white overflow-x-auto">
                {JSON.stringify(chartConfigs[selectedChart].data.dataLabels, null, 2)}
              </pre>
            </div>
          )}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 bg-primary-white/10 text-xs text-primary-white rounded">
                {chartConfigs[selectedChart].data.data?.length || 0} Data Points
              </span>
              {chartConfigs[selectedChart].data.dataLabels?.enabled && (
                <span className="px-2 py-1 bg-primary-white/10 text-xs text-primary-white rounded">
                  Data Labels Enabled
                </span>
              )}
              {chartConfigs[selectedChart].data.colorAxis?.stops && (
                <span className="px-2 py-1 bg-primary-white/10 text-xs text-primary-white rounded">
                  Custom Color Stops: {chartConfigs[selectedChart].data.colorAxis.stops.length}
                </span>
              )}
              {chartConfigs[selectedChart].data.borderWidth !== undefined && (
                <span className="px-2 py-1 bg-primary-white/10 text-xs text-primary-white rounded">
                  Border Width: {chartConfigs[selectedChart].data.borderWidth}px
                </span>
              )}
            </div>
          </div>
