  ...
  const chartOptions: Highcharts.Options | undefined = useMemo(() => {
    ...
    const validatedSeries = allSeries.map((series) => {
      ...
      if (typeof seriesYAxis === 'number' && seriesYAxis > maxYAxisIndex) {
        console.warn(`Series "${series.id}" references yAxis ${seriesYAxis} but only ${yAxes.length} axes exist. Clamping to ${maxYAxisIndex}`)
        return {
          ...series,
          yAxis: maxYAxisIndex
        }
  ...
  return (
    <div className="flex h-full w-full flex-col items-start">
      <div className="relative h-full w-full flex-1" ref={chartContainerRef}>
        ...
        ) : typeof chartOptions === "undefined" || !assetId || isTimeframeSwitching ? (
          <div className="h-full w-full rounded-3xl">
            <div className="flex h-full items-center justify-center text-gray-500">
              {!assetId ? "Please select an asset" : isTimeframeSwitching ? "Switching timeframe..." : "No data found!"}
            </div>
          </div>
        ) : (
          <>
