...
import { TearsheetTable } from '@epochlab/epoch-dashboard'
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        <TearsheetTable
          table={{
            columns: tableData.headers.map((h, i) => ({ name: h, type: tableData.columnTypes[i] })),
            data: { rows: tableData.rows.map(row => ({ values: row })) }
          }}
        />
      </div>
